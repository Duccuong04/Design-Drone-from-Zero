...
void ReadJoysticksAndButton(void)
{
    ...
    int mapYaw     = (adc_buffer[3] * 1000) / 4070 + 1000;
    int mapThrottle= (adc_buffer[2] * 1000) / 4070 + 1000;
    int mapRoll    = (adc_buffer[1] * 1000) / 4070 + 1000;
    int mapPitch   = (adc_buffer[0] * 1000) / 4070 + 1000;

    /* ====== Encode Tx_data giống bên RX mong muốn ====== */

    // Byte 0: throttle lên/xuống/giữ
    if (mapThrottle > 1900)       Tx_data[0] = 1;   // lên
...
void SSD1306_OLED()
{
	...
	SSD1306_GotoXY (0,0);
	SSD1306_Puts ("Game Pad", &Font_11x18, 1);
	SSD1306_GotoXY (0,20);
	SSD1306_Puts ("Throttle", &Font_11x18, 1);
	SSD1306_UpdateScreen(); //display
}

void SSD1306_ShowThrottle(int throttleValue)
{
    char buffer[20];
    sprintf(buffer, " %d", throttleValue);

    SSD1306_GotoXY(0, 40);  // Dòng thứ 3 (y=40)
...
int main(void)
{
 ...
 // Set ở chế độ Tx cho GamePad
  ...
  while (1)
  {
	  ...
	  SSD1306_ShowThrottle(Tx_data[0]);
