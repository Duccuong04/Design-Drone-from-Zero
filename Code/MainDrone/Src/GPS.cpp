...
/*
 * Lưu thông tin LED báo trạng thái GPS.
 */
...
/*
 * Thiết lập hằng số P, D cho Roll.
 */
...
/* tối ưu tốc độ và băng thông UART.
 * Gửi lệnh UBX để:
 * Tắt gói GPGSV (giảm dữ liệu không cần thiết).
 * Đặt refresh 5Hz.
 * Đổi baudrate từ 9600 → 57600.
 * Re-init UART STM32 sang 57600.
 */
...
/*
 * Đọc UART từng byte, lưu vào incommingMessage[].
 * Khi thấy $ → bắt đầu bản tin mới.
 * Khi thấy * → kết thúc bản tin.
 * Nếu là $GPGLL → báo mất fix → reset dữ liệu.
 * Nếu là $GPGGA và fix valid → giải mã lat, lon, số vệ tinh.
 * Thực hiện nội suy dữ liệu GPS vì GPS chỉ 5Hz, nhưng vòng lặp control cần 50Hz.
 * Cập nhật gpsRollAdjust, gpsPitchAdjust theo PID + góc yaw.
 * Nếu số vệ tinh < 8 → bật LED báo GPS yếu.
 * Nếu mất tín hiệu lâu >1s → báo lỗi, hạ flight mode.
 * Nếu vào FlightMode ≥ 3 → lưu waypoint, tính sai số để dẫn đường.
 */
