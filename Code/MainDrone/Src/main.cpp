...
// Biến log UART 4 động cơ
char txBuf[64];

// biến static để nhớ thời gian vòng lặp trước
...
//	else
//	{
//		if ((HAL_GetTick() - lastRxTime) > 100) // >100ms không có gói
//		{
//
//		// Mất tín hiệu, tự động giảm throttle
...
int main(void)
{
  ...
  DWT_Delay_Init();
  lastCycle = DWT->CYCCNT;        // khởi tạo mốc ngay sau enable
  //BMP180_Start();
...
//   pidForHorizontal.setKgainConstantYaw(1.4, 0.02 , 0, 400);
   ...
   // cái này khá ok
//   pidForHorizontal.setKgainConstantRoll(1.5, 0.018 , 20, 400);
//   pidForHorizontal.setKgainConstantPitch(1.5, 0.018, 20, 400);
//   pidForHorizontal.setKgainConstantYaw(0.8, 0.015 , 0, 400);

//      pidForHorizontal.setKgainConstantRoll(0.4, 0.3 , 0.06, 400);
//      pidForHorizontal.setKgainConstantPitch(0.4, 0.3, 0.06, 400);
//      pidForHorizontal.setKgainConstantYaw(0.3, 0.2 , 0, 400);

   	   	 pidForHorizontal.setKgainConstantRoll(0.4f, 0.12f, 0.06f, 400);
   	   	 pidForHorizontal.setKgainConstantPitch(0.4f, 0.12f, 0.06f, 400);
         pidForHorizontal.setKgainConstantYaw(0.3, 0.2 , 0, 400);
...
//	  }
	 ...
	 // CHUYỂN StartStopTakeOff LÊN TRƯỚC để xử lý trạng thái arm ngay vòng lặp này
	 StartStopTakeOff();


	 // Chỉ tính PID khi motors đã được khởi động (stateMachine == 2)
	 // Nếu chưa arm thì reset integral / output để chống wind-up và tránh spike khi arm
	 if (stateMachine == 2)
	 {
	     ...
	     uint32_t diff = now - lastCycle;
	     lastCycle = now;
	     float dt = (float)diff / (float)CPU_FREQ_HZ;

	     // bảo vệ: nếu dt bất thường do tràn hoặc lần đo đầu, gán fallback
	     if (dt <= 0.0f || dt > 0.1f) {
	         dt = 0.004f; // ví dụ fallback 4ms (tùy bạn)
	     }


	     // Outer loop: dựa trên RC + góc IMU
	     //pidForHorizontal.calculateOuterLoop(gy86.angleRoll, gy86.anglePitch,dt);
	     pidForHorizontal.calculatePID(dt);

	 }
	 else
	 {
	     // reset anti-windup tạm: xóa integral + previous error + output
	     pidForHorizontal.roll.ImemValue = 0.0f;
	     pidForHorizontal.pitch.ImemValue = 0.0f;
	     pidForHorizontal.yaw.ImemValue = 0.0f;

	     pidForHorizontal.roll.previousError = 0.0f;
	     pidForHorizontal.pitch.previousError = 0.0f;
	     pidForHorizontal.yaw.previousError = 0.0f;

	     pidForHorizontal.roll.outputValue = 0.0f;
	     pidForHorizontal.pitch.outputValue = 0.0f;
	     pidForHorizontal.yaw.outputValue = 0.0f;
	 }
...
//	 StartStopTakeOff();
	...
	else
	{
	 ...
	 CHANGE_SPEED_MOTOR_FRONT_LEFT_CW(escFrontLeftCW);
	 CHANGE_SPEED_MOTOR_FRONT_RIGHT_CCW(escFrontRightCCW);
	 CHANGE_SPEED_MOTOR_REAR_LEFT_CCW(escRearLeftCCW);
	 CHANGE_SPEED_MOTOR_REAR_RIGHT_CW(escRearRightCW);

	 // Format: esc1,esc2,esc3,esc4\n
	 int len = snprintf(txBuf, sizeof(txBuf), "%d,%d,%d,%d,%.2f,%.2f,%.2f\r\n",
	                    escFrontLeftCW,
	                    escFrontRightCCW,
	                    escRearLeftCCW,
	                    escRearRightCW,pidForHorizontal.pitch.outputValue,pidForHorizontal.roll.outputValue,pidForHorizontal.yaw.outputValue);
