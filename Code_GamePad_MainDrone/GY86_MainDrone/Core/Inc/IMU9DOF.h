...
public:
	...
	*/
	...
	/*
	 * Tính độ lệch góc giữa 2 giá trị heading (hữu ích khi so sánh hướng hiện tại và hướng mục tiêu).
	 * Kết quả nằm trong khoảng -180° đến 180°.
	 */
	float CourseDeviation(float theValue1, float theValue2);

	/*
	 * Tính toán Roll – Pitch – Yaw từ dữ liệu cảm biến.
	 */
	void ReadAngleRPY();

	/*
	 * Xử lý dữ liệu Gyro để tính tốc độ quay (deg/s) và tích hợp theo thời gian để ra góc.
	 */
	void CalculateGyroInput();

	/*
	 * Đặt IMU yên, đo giá trị Gyro nhiều lần.
	 */
	void CalibGyro();

	/*
	 * Hiệu chuẩn HMC5883L bằng cách xoay IMU theo nhiều hướng → tìm offset & scale cho la bàn.
	 * Lưu vào compassOffset_X/Y/Z, compassScaleY/Z.
	 */
	void CalibCompass();

	/*
	 * Đặt IMU trên mặt phẳng ngang.
	 * Đo giá trị Accel → tính offset cho Pitch và Roll (accPitchCalValue, accRollCalValue).
	 */
	void CalibLevel(uint8_t* theError);

	/*
	 * Dùng dữ liệu Accel Z sau khi lọc → tính gia tốc thẳng đứng
	 * Tích hợp ra accelAltIntegrated (phần tử thay đổi độ cao).
	 * Kết hợp với barometer (MS5611) để ổn định đo cao độ.
	 */
	...
	/*
	 * Khởi động tất cả sensor (MPU6050, HMC5883L, MS5611).
	...
	/*
	 * Cấu hình từng cảm biến (register setup).
	 * SetupAutomaticReadCompass(): có thể bật chế độ I2C bypass của MPU6050 để đọc trực tiếp compass.
	 * Chọn độ phân giải (±2g, ±4g, ±8g, ±16g cho Accel; ±250, 500, 1000, 2000°/s cho Gyro; ±0.88 ~ ±8.1 Gauss cho Magn).
	 */
	...
	/*
	 * Kiểm tra thiết bị MPU6050, HCL, MS
	 * Đọc WHO_AM_I register của từng cảm biến để chắc chắn rằng thiết bị có kết nối.
	 */
	...
	/*
	 * Tinh chỉnh nhỏ để giữ drone cân bằng hoàn hảo.
	 * theRateRoll, theRatePitch là giá trị điều chỉnh bổ sung dựa trên PID.
	 */
	void setLevelAdjust(uint8_t theRateRoll, uint8_t theRatePitch);
private:

	/*
	 * → Dùng để lưu lại trạng thái đã hiệu chuẩn con quay hồi chuyển (Gyro) hay chưa.
	 */
	int16_t IsAlreadyCalibGyro;

	/*
	 * Cờ đã hiệu chuẩn la bàn (Compass/HMC5883L) hay chưa.
	 */
	bool isAlreadyCalibCompass;

	/*
	 * Xác định có đang trong quá trình hiệu chuẩn cân bằng (Level Calibration) không.
	 */
	bool isOnLevelCalib;

	/*
	 * Đây là các giá trị offset của cảm biến từ trường (Magnetometer HMC5883L), thường dùng để loại bỏ sai số cứng (hard iron distortion).
	 */
	int16_t compassOffset_X, compassOffset_Y, compassOffset_Z;

	/*
	 * Thực tế khi hiệu chuẩn la bàn, ta vẽ dữ liệu từ trường thành hình elip → sau đó scale để đưa về hình tròn.
	 */
	float compassScaleY, compassScaleZ;

	/*
	 * Xác định sector của Flash STM32 dùng để lưu dữ liệu hiệu chuẩn.
	 */
	uint8_t sectorFlash;
	...
	/*
	 * Mảng tạm để ghi/đọc dữ liệu hiệu chuẩn từ Flash (ví dụ: offset gyro, offset compass, scale).
	 */
	int16_t dataFlash[6];
};
